...
{
  GLWidget &g = *glwidget();
  for(unsigned int i=0; i<vertexBuffers.size(); i++) // for each buffer (that is, for each object)
    {
      g.glDeleteBuffers(vertexBuffers.size(),  &vertexBuffers[i]);
...
bool DrawVBO::drawScene()
{
    ...
    // Pinta por cada objeto, NO TIENE SENTIDO.
	for(unsigned int i=0; i<vertexBuffers.size(); i++) // for each buffer (that is, for each object)
	{
...
// Por cada vertice te devuelve a que caras da.
...
{  
    ...
    assert(vertexBuffers.size() == currentObject);
    vertexBuffers.push_back(coordBufferID);
    ...
    g.glBindBuffer(GL_ARRAY_BUFFER, vertexBuffers[currentObject]);
    ...
    Q_EXPORT_PLUGIN2(drawSmooth, DrawVBO)   // plugin name, plugin class
#endif
